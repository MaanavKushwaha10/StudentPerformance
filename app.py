    ...
    if os.path.exists(model_path):
        try:
            with open(model_path, "rb") as f:
                return pickle.load(f), True, None
        except ModuleNotFoundError as e:
            return None, False, f"Missing module: {e}. Add it to requirements.txt."
        except Exception as e:
            return None, False, f"Failed to load model: {e}"
    return None, False, None
 
 
model_pipeline, model_loaded, model_error = load_model()
...
if model_error:
    st.error(f"⚠️ **Model load error:** {model_error}")
